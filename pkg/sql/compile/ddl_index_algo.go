...
func (s *Scope) handleIvfIndexEntriesTable(c *Compile, indexDef *plan.IndexDef, qryDatabase string, originalTableDef *plan.TableDef,
	...
	originalTableWithNormalizedSkSql := fmt.Sprintf("(select "+
		"%s as `%s`, "+
		"normalize_l2(`%s`.`%s`) as `__mo_index_tbl_norm_vec`, "+
		"`%s`.`%s`"+
		" from `%s`.`%s`) as `%s`",
	...
	/*
			Sample SQL:
			INSERT INTO `a`.`entries_tbl`(`__mo_index_centroid_fk_version`, `__mo_index_centroid_fk_id`, `__mo_index_pri_col`, `__mo_index_centroid_fk_entry`)
	...
	*/
	...
	mappingSQL := fmt.Sprintf("%s "+
		...
		"%s ,"+
		"`%s`.`%s` "+
		"from %s CROSS JOIN %s group by %s, `%s`.`__mo_index_centroid_version`, `%s`.`%s`;",
		insertSQL,
